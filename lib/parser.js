...
var queryStart = /^\s*(select|update|insert|delete|create|drop|replace)\s*$/i;
var helperStart = /^\s*\{#\s*(if|set|where|trim|foreach)(\s+.*\s*)?}\s*$/;
var helperEnd = /^\s*\{\/\s*(if|set|where|trim|foreach)\s*}$/;
...
Parser.prototype.parse = function(input){
    ...
    while(context.currentPos < length){
        ...
        if(!context.startQuery && context.text.length > 3){
        ...
        if (context.currentChar === '{') {
            if(!context.startQuery){
                throw new ParseError(input, context.currentPos - 1, ['select', 'update', 'insert', 'delete', 'create', 'drop'],
                    'unexpected \'{\'');
            }
