...
function _getExecutable(originalContext){
    return function execute(context, callback) {
        originalContext.holdRealStackTrace(execute);
        if(context.connector){
            originalContext.connector = context.connector;
        }
        if(context.connection){
            originalContext.connection = context.connection;
        }
        ...
        if(!context) {
            callback('Invalid context');
            _logging(null, 'Invalid context');
        } else if(!originalContext.connection) {
            originalContext.connector.beginConnection(function (err, newConn) {
                if (err) {
                    callback(new DBError(err, 'An error occurred while creating connection'));
                    _logging(originalContext.daoInfo, err);
                } else {
                    originalContext.connection = newConn;
                    originalContext.options.closeConnection = true;
                    //context.options.useContextOnly = false;
                    _execute(originalContext, callback);
                }
            });
        } else {
            originalContext.options.closeConnection = false;
